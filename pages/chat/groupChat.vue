<template>
	<view>
	 <scroll-view  v-if="infoList.length > 0" style="display: flex;flex-direction: column;height: 100vh;padding-top: 20rpx;"  class="container" scroll-y="true" 
		:scroll-into-view="scrollToView" ref="scrollContainer">
	      <view v-for="(item,index)  in infoList" :key="index" :id="'msg' + item.id" style="display: flex;margin-bottom: 20rpx" ref="myView">
	        <view v-if="item.from !== userInfo.phone" style="margin-right: auto;margin-left: 10rpx;display: flex;align-items: center;">
	          <img :src="`${imgUrl}/common/download?name=${item.avatar}`"  style="width: 120rpx;height: 120rpx;border-radius: 50%;">
			  
	          <view v-if="item.ifImg===0" style=" display: flex;flex-direction: column;font-size: 15px;margin-left: 20rpx;border-radius: 10rpx;">
				<view style="color: #808080;margin-right: auto;margin-left: 10rpx;">{{item.nickName}}</view>
				<view style="background-color: #f5f5f5; padding: 20rpx;border-radius: 10rpx;"> {{item.content}}</view>
			  </view>
	         <view style="display: flex;flex-direction: column;" v-if="item.ifImg===1">
	         			<view style="color: #808080; margin-right: auto;margin-left: 20rpx;">{{item.nickName}}</view>
	         		<view  style="border-radius: 10rpx;"><image mode="aspectFill" @click="previewImage(item.img)" style="margin-right: 20rpx;width: 300rpx;height: 300rpx" :src="`${imgUrl}/common/download?name=${item.img}`"></image></view>	
	         </view>
	        </view>
	        <view v-if="item.from === userInfo.phone" style="margin-left: auto;margin-right: 10rpx;display: flex;align-items: center;">
	          <view style="display: flex;flex-direction: column;font-size: 15px;margin-right: 20rpx;" v-if="item.ifImg===0">
			  <view style="color: #808080;margin-left: auto;margin-right: 10rpx;">{{item.nickName}}</view>
			  <view style="background-color: #5cd375; padding: 20rpx;border-radius: 10rpx;"> {{item.content}}</view>
			  </view>
	          <view style="display: flex;flex-direction: column;" v-if="item.ifImg===1">
					  <view style="color: #808080; margin-left: auto;margin-right: 20rpx;">{{item.nickName}}</view>
					  <view  style="border-radius: 10rpx;"><image mode="aspectFill" @click="previewImage(item.img)" style="margin-right: 20rpx;width: 300rpx;height: 300rpx" :src="`${imgUrl}/common/download?name=${item.img}`"></image></view>	
	          </view>
...
<script>
	import {sendToGroup} from'@/api/chatGroup.js'
	...
	export default {
		...
		onLoad(option) {
			...
			const groups = JSON.parse(decodeURIComponent(option.groups));
			this.groupId = groups[0].groupId 
			const filteredGroups =groups.filter(group => group.ifImg !== 0 || group.content !== null);
			...
			this.socket = uni.connectSocket({
			  url: config.WS_SERVER_URL + '/imserver/group/' + this.groupId,
			  success: res => {
			    console.log(res, 'res');
			    console.info('创建连接成功');
			  }
		...
		methods: {
			sendMsg(){
				if(this.info.content!=='' || this.info.img!==''){
					...
					let that = this
					sendToGroup(this.info).then(
						res =>{
								console.log(res);
								this.infoList.push(sendInfo)
								console.log(this.infoList,'ti');
					...
					success: (chooseImageRes) => {
						...
						uni.uploadFile({
							...
							success: (uploadFileRes) => {
								...
								let that = this 
								sendToGroup(this.info).then(res =>{
									if(res.code ==1){
